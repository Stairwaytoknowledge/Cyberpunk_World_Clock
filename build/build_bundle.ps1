...
# Icons are gitignored (they're regenerated by generate_icons.py), so if
# build_bundle.ps1 is running against a fresh clone we need Pillow plus a
# generator run to produce assets/icon.ico and the Store tiles.  The
# Desktop shortcut that Install.bat creates later points at assets/icon.ico,
# so this MUST happen before the "Copy app files" step.
# Pillow is a build-time dependency only; uninstall it right after so it
# doesn't bloat the shipped runtime.
& "$runtime\python.exe" -m pip install --no-warn-script-location Pillow | Out-Host
& "$runtime\python.exe" (Join-Path $root "build\generate_icons.py") | Out-Host
& "$runtime\python.exe" -m pip uninstall -y Pillow | Out-Host
